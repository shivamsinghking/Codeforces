...
public class Main {
   ...
   static Boolean[][] dp;
   static boolean isPossible(int[] arr, int index, int sum){
      ...
      if(dp[index][sum] != null){
        // out.println(" ii< > "  + index + " "  + sum + " " + dp[index][sum]);
        return dp[index][sum];
      }
      
      boolean a = isPossible(arr, index+1, sum - arr[index]);
      boolean b = isPossible(arr, index+1, sum);
