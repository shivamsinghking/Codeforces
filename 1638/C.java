...
public class Main {
    ...
    public static void solve() {
        ...
        //  out.println(Arrays.toString(suffixMin));

         DSU d = new DSU(n+1);

         for(int i = 1; i < n; i++){
             int parent = d.findParent(arr[i-1]);
             if(arr[i] < parent){
                 d.Union(arr[i], parent);
             }else{
                 int min = suffixMin[i];
                 if(min < parent){
                     d.Union(parent, arr[i]);
                 }
             }
         }

        //  out.println(Arrays.toString(d.parent));
         HashSet<Integer> set = new HashSet<>();
         for(int i = 0; i < n; i++){
             int p = d.findParent(arr[i]);
             set.add(p);
         }
