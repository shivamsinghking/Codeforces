...
public class Main {
    ...
    public static void solve() {
      ...
      for(int i = 1; i < n; i++){
          long prev = arr[i-1];
          if(arr[i] - prev <= x) continue;

          long req = ceil(arr[i] - prev, x) - 1;
          // while(arr[i] - prev > x){
          //    prev += x;
          //    req++;
          // }

          // out.println(" req "  + req  + " " + arr[i] + " " + prev + " " + k);
          if(req <= k){
            q.add(req);
            k  -= req;
            continue;
    ...
    // static void ruffleSort(long[] a) {int n=a.length;Random r=new Random();for (int i=0; i<a.length; i++) {long oi=r.nextLong(n), temp=a[i];a[i]=a[oi];a[oi]=temp;}Arrays.sort(a);}
