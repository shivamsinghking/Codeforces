...
public class Main {
   ...
   static long query(long r, long lcm, int max){
        if(r < max) return 0;
        ...
        if(div != 0){
          total -= (div - 1)*max;
          total -= Math.min(max, (r - pro + 1));
        }
        return total - Math.min(r, max - 1);
    }
    ...
    public static void solve() {
         ...
         for(int i = 0; i < q; i++){
           ...
           long ll = (l > 1) ? query(l-1, com, max) : 0;
           long rr = query(r, com, max);
           out.print((rr - ll) + " ");
         }
         out.println();
    }
