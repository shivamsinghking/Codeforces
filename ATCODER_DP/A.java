...
public class Main {
    ...
    static Integer[] dp;
    static int solve(int index, int[] cost){
      if(index == cost.length  - 1) return 0;
      if(index >= cost.length) return 0;

      if(dp[index] != null) return dp[index];

      int a = Integer.MAX_VALUE - 100000;
      if(index+2 < cost.length){
        a = Math.abs(cost[index] - cost[index + 2]) + solve(index+2, cost);
      }

      int b = Integer.MAX_VALUE - 100000;
      if(index+1 < cost.length)
      {
        b = Math.abs(cost[index] - cost[index+1]) + solve(index+1, cost);
      }
