...
public class Main {
    ...
    static Integer[][] dp;
    // limit : 30
    static int helper(int index, String[] words, int[] cost, int limit){
      if(limit <= 0) return 0;
      if(index >= words.length) return 0;

      int a = Integer.MIN_VALUE;

      if(dp[index][limit] != null) return dp[index][limit];
      if(limit - (words[index].length()) > 0){
        a = cost[index] + helper(index+1, words, cost, limit - words[index].length() - 1);
      }

      int b = helper(index+1, words, cost, limit);
      return dp[index][limit] = Math.max(a, b);
    }
