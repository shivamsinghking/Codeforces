...
public class Main {
    ...
    static Integer[][] dp;
    static int solve(int index, int[][] arr, int prevIndex){
      ...
      for(int i = 0; i < 3; i++){
        if(i != prevIndex){
          int v = arr[index][i] + solve(index+1, arr, i);
          bestCase = Math.max(bestCase, v);
        }
      }

      return dp[index][prevIndex + 1] = bestCase;
    }
