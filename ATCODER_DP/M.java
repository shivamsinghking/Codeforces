...
public class Main {
    ...
    // static long helper(int index, int k, int[] arr){
    //   if(index >= arr.length && k > 0) return Integer.MAX_VALUE;
    //   if(index >= arr.length && k <= 0) return 1;

    //   if(k <= 0) return 1;

    //   if(dp[index][k] != null){
    //     //   out.println(" 00> " + index + " " + k);
    //     return dp[index][k];
    //   }

    //   long ans = 0;
    //   for(int i = 0; i <= Math.min(k, arr[index]); i++){
    //     long v = helper(index+1, k - i, arr);
    ...
    public static void solve() {
         ...
         for(int i = 1; i < n; i++){
             ...
             // dp[i-1][1] + dp[i - 1][2] + [i-1][3] ... //
             // [i-1][4] + [i-1][3] + dp[j - arr[i]]
             ...
             for(int j = 0; j <= k; j++){
                 if(j > arr[i]){
                    dp[i][j] = (prefix_sum[j] + MOD - prefix_sum[j - arr[i] - 1])%MOD;
                 }else{
                     dp[i][j] = prefix_sum[j];
                 }
             }
