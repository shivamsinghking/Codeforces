...
public class Main {
    ...
    int[] prefix;
    int helper(int s, int e, int[] arr, int piles, int k) {
      if (s > e)
        return Integer.MAX_VALUE;

      int len = e - s + 1;
      if (piles == 1 && ((len == k || len == 1))) {
      ...
      if (piles == 1) {
        ...
        for (int i = s; i < e; i += k - 1) {
          int v = helper(s, i, arr, 1, k);
          int u = helper(i + 1, e, arr, k - 1, k);
          if (v != Integer.MAX_VALUE && u != Integer.MAX_VALUE) {
            min = Math.min(min, v + u);
          }
        }
        return min;
      }
      ...
      for (int i = s; i < e; i += k - 1) {
        int v = helper(s, i, arr, 1, k);
        int u = helper(i + 1, e, arr, piles - 1, k);
        if (v != Integer.MAX_VALUE && u != Integer.MAX_VALUE) {
          min = Math.min(min, v + u);
        }
      }
