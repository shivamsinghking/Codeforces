...
public class Main {
    ...
    static Integer[][] dp;
    static int helper(int index, int token, int[] arr){
      if(index >= arr.length) return 0;

      if(dp[index][token] != null) return dp[index][token];
      // token == true => bob
      if(token == 1){
          int a = arr[index] + helper(index+1, 0, arr);
          int b = helper(index+1, 1, arr);
          return dp[index][token] = Math.max(a, b);
      }else{
        int a = helper(index+1, 1, arr);
        int b = arr[index] + helper(index+1, 0, arr);
        return dp[index][token] = Math.min(a, b);
      }
    }
