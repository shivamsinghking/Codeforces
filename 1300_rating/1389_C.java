...
public class Main {
    ...
    public static void solve() {
      ...
      for(int i = 0; i <= 9; i++){
        for(int j = 0; j <= 9; j++){
          boolean turn = true; // if true => search i, false search j
          int cnt1 = 0;
          int cnt2 = 0;
          int remove = 0;
          int search = i;
          ...
          // out.println(" start ==> ");
          for(int k = 0; k < n; k++){
            // out.println("remove ==> " + remove + " " + cc[k] + " " + search);
              if((cc[k] - '0') == search){
                  remove += (k - prev - 1);
                  prev = k;
                  if(search == i) cnt1++;
          ...
          remove += (n - prev - 1);
          // out.println(" i " + i + " " + j + " ");
          // out.println("cnt1 " + cnt1 + " " + "cnt2 " + " "  + cnt2 + " remove " + remove);
          if(i != j && (cnt1 == cnt2) && (cnt1 > 0 && cnt2 > 0)){
            min = Math.min(remove, min);
          }else if(i == j && (cnt1 > 0 || cnt2 > 0)){
            min = Math.min(remove, min);
          }
        }
      }
