...
public class Main {
    ...
    public static void solve() {
      ...
      for(int i = 1; i < n; i++){
         long total_edges = 0L;
         total_edges += c[i] - 1 + Math.abs(b[i] - a[i]) + 2;
         max = Math.max(max, total_edges);
         int j = i+1;
         for(; j < n; j++){
           if(a[j] == b[j]) break;
           long self_total = (c[j] - 1 + Math.abs(b[j] - a[j]) + 2);
           total_edges += c[j] - 1 + 2;
           total_edges -= Math.abs(b[j] - a[j]);
           if(self_total > total_edges){
             total_edges = self_total;
           }
          //  long res = Math.max(total_edges, self_total);
           max = Math.max(max, total_edges);
         }
         i = j - 1;
         max = Math.max(total_edges, max);
      }
      out.println(max);
    }
