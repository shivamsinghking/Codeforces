...
public class Main {
    ...
    public static void solve() {
        ...
        int sum2 = 0;
        for(int i: brr) sum2 += i;

        int range = 2*100000 + 10;
        dp = new Integer[n+1][range+offset];

        int ans = find(n - 1, arr, brr, 0, 0);
        long total = 0L;
        for(int i = 0; i < n; i++){
            total += (arr[i]*arr[i] + brr[i]*brr[i])*(n - 1);
        }
        total += 2*ans;
        out.println(total);
    }
    ...
    static int find(int index, int [] arr, int[] brr, int s1, int s2){
        if(index < 0){
          return 0;
        }

        if(dp[index][s2+offset] != null){
            // out.println("fdf");
            return dp[index][s2+offset];
        }

        int sum1 = (s1+s2)/2;
        int sum2 = (s1 - s2)/2;

        // out.println(" index " + index + " "  + sum1 + " " + sum2 );
        int val1 = arr[index]*sum1 + brr[index]*sum2;
        int ans1 = val1 + find(index - 1, arr, brr, sum1+arr[index] + sum2+brr[index], sum1+arr[index] - (sum2+brr[index]));

        // swap
        int val2 = arr[index]*sum2  + brr[index]*sum1;
        int ans2 = val2 + find(index - 1, arr, brr, sum1+brr[index] + sum2 + arr[index], sum1+brr[index]- (sum2 + arr[index]));
        return dp[index][s2+offset]= Math.min(ans2, ans1);
    }
