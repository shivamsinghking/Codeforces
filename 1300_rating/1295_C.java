...
public class Main {
   ...
   static int check(int start, int n,  HashMap<Character,TreeSet<Integer>> map, char c){

      if(start >= n) return n;
     
      TreeSet<Integer> s = map.get(c);
      Integer index = s.ceiling(start);
      if(index == null) return n;
      else return index;
    }
    public static void solve() {
      ...
      for(int i = 0; i < ss.length; i++){
        if(map.containsKey(ss[i])){
          TreeSet<Integer> set = map.get(ss[i]);
          set.add(i);
          map.put(ss[i], set);
        }else{
      ...
      for(int i = 0; i < tt.length; i++){
        int start = 0;
        start = check(start, ss.length, map, tt[i]);
        if(start < ss.length){
            int j = i+1;
            while(j < tt.length){
             int val =  check(start+1, ss.length, map, tt[j]);
             if(val < ss.length){
               start = val;
               j++;
             }else{
